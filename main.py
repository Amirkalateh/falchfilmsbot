...
licence_code = "falch2024"
my_bot_token = os.environ['YOUR_API_KEY']



def save_user_message_to_db(update: Update) -> None:
  conn = sqlite3.connect('conversation_log.sql')
  cursor = conn.cursor()
  ...
  cursor.execute(
      '''INSERT INTO messages (user_id, telegram_id, message) VALUES (?, ?, ?)''',
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  await update.message.reply_html(
      rf"Hi {user.mention_html()}! Please provide the license code.",
      reply_markup=ForceReply(selective=True))
  save_user_message_to_db(update)
...
async def stylize(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  test = "Test, trinity Your Log :" + user_message
  if user_message is None:
    await update.message.reply_text("Please send an image to stylize.")
    return

  img = Image.new('RGB', (1000, 400), color=(73, 109, 137))
  d = ImageDraw.Draw(img)
  fnt = ImageFont.load_default()
  d.text((50, 90), test, font=fnt, fill=(511, 511, 0))
